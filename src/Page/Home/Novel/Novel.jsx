...
const Novel = () => {
    ...
    useEffect(() => {
        fetch('http://localhost:5000/bestnovel')
            .then(res => res.json())
            .then(data => setnovelBooks(data))
    ...
    return (
        <>
            <h1 className='text-center text-4xl'>Best Seller novel Books</h1>
            <div className='relative flex items-center'>
                ...
                <div id='slider' className='w-full h-[400px] overflow-x-scroll overflow-y-hidden scroll whitespace-nowrap scroll-smooth scrollbar-hide'>
                    {novelBooks.map((novelBook) => (
                        <div key={novelBook._id} className='inline-block'>
                            <Link to={`/bestnovel/${novelBook._id}`}>
                                <img className='w-[250px] h-[300px]  mr-2 ml-2 inline-block p-2 cursor-pointer hover:scale-105 ease-in-out duration-300' src={novelBook.img} alt='books' />
                            </Link>
                            <h2 className='w-[250px] h-[150px] text-xl text-center overflow-hidden whitespace-normal'>
                                <Link to={`/novel/${novelBook._id}`} className='text-xl'>{novelBook.name}</Link>
                            </h2>
                        </div>
                    ))}
                </div>
            ...
            <div>
                <h1 className='text-center text-4xl'>All novel Books</h1>        </div>
