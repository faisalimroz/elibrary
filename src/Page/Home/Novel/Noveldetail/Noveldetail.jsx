...
const Noveldetail = () => {
    ...
    useEffect(() => {
      fetch(`http://localhost:5000/novel/${nlid}`)
        .then(response => {
          if (!response.ok) {
    ...
    useEffect(() => {
      const fetchData = async () => {
          try {
              const response = await axios.get(`http://localhost:5000/order/${user.email}/${nlid}`);
          ...
          <div id='' className="blog  mx-auto ">
            <div className="blog-div">
              ...
              <button type="button" className="bg-yellow-300 mt-1 mb-3 rounded-lg  p-3">
                <a target="_blank" href={paymentData?.id === nlid ? novel.pdflink : '#'}>
                  PDF
                </a>
              </button>
            ...
            <form onSubmit={handleSubmit} className="vertical-form">
              ...
              <div className="form-group">
                <p className='text-1xl'>Price per Member: ${members * novel.price}</p>
