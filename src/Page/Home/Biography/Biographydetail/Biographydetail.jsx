...
const Biographydetail = () => {
  ...
  useEffect(() => {
    fetch(`http://localhost:5000/biography/${bid}`)
      .then(response => {
        if (!response.ok) {
  ...
  useEffect(() => {
    const fetchData = async () => {
        try {
            const response = await axios.get(`http://localhost:5000/order/${user.email}/${bid}`);
        ...
        <div id='' className="blog  mx-auto ">
          <div className="blog-div">
            ...
            <button type="button" className="bg-yellow-300 mt-1 mb-3 rounded-lg  p-3">
              <a target="_blank" href={paymentData?.id === bid ? biography.pdflink : '#'}>
                PDF
              </a>
            </button>
